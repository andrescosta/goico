...
func TestInitRouteErrors(t *testing.T) {
	...
	_, err := New(
		...
		WithAddr(localhost),
		WithName("listener-test"),
		WithInitRoutesFn[*ServiceOptions](func(_ context.Context, _ *mux.Router) error {
...
func TestSamePort(t *testing.T) {
	...
	svc1, err := New(
		...
		WithAddr(localhost),
		WithName("listener-test"),
		WithInitRoutesFn[*ServiceOptions](func(_ context.Context, r *mux.Router) error {
			r.HandleFunc("/", func(_ http.ResponseWriter, _ *http.Request) {}).Schemes("http")
			return nil
		}),
	)
	if err != nil {
